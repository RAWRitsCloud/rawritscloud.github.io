...
async function scrapeMicrosoft(page) {
  ...
  const credentialsTab = page
    ...
    .or(page.getByRole('button', { name: /credentials/i }))
    .first();
  ...
  const records = await page.evaluate(() => {
    ...
    /*
     * Remove large wrapper elements where a smaller matching card exists
     * inside them.
     */
...
async function scrapeCredly(page) {
  ...
  if (links.length === 0) {
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, 'credly.png'),
      fullPage: true,
    });
  ...
  return [
    ...new Map(
      badges.map((item) => [key(item.name), item]),
    ).values(),
  ];
}
...
function existingCertificateMap(data) {
  ...
  for (const section of data.sections || []) {
    for (const certificate of section.certs || []) {
      map.set(key(certificate.name), certificate);
    }
  }
...
function toDisplayCertificate(item, existing) {
  const certificate = {
    name: item.name,
    level:
      existing?.level || inferLevel(item.name),
    date:
  ...
  if (item.expires) {
    certificate.expires = shortDate(item.expires);
  }

  /*
   * Keep your existing locally stored badge image wherever
   * possible. New certificates can use the image supplied by
   * Microsoft Learn or Credly.
   */
  if (existing?.image) {
...
function buildData(current, microsoft, credly) {
  ...
  /*
   * Microsoft Learn wins when the same certification exists
   * in both places. Credly-only Microsoft badges are treated
   * as historical.
   */
  ...
  for (const item of activeMicrosoft) {
    const title = microsoftSectionTitle(item.name);
  ...
  /*
   * Preserve any unrelated manually managed sections.
   */
  ...
  /*
   * Anything without an automated source marker is considered
   * one of your manually maintained legacy qualifications.
   */
  const manualLegacy = (
    current.legacy || []
  ).filter(
  ...
  const legacyKeys = new Set(
    manualLegacy.map((item) => key(item.name)),
  );
